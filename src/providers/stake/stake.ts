...
export class StakeProvider {
  ...
  constructor(
    ...
    this.jwanContractToken = new this.web3.eth.Contract(
      JWAN_TOKEN_ABI,
      this.jwanTokenAddress
    );
    this.jwanContractStake = new this.web3.eth.Contract(
      JWAN_STAKE_ABI,
      this.jwanStakeAddress
    );
    this.depositData = this.jwanContractStake.methods
      .deposit(this.etherToWei('100'))
      .encodeABI();
  ...
  public getIndexesDeposits(userJwanAddress) {
    return this.callMethod(this.jwanContractStake, 'indexesForUser', [
      userJwanAddress
    ]);
  }

  private getIndexesDepositsArray(
    userJwanAddreses: string[]
  ): Array<Promise<string>> {
    let promises: Array<Promise<string>> = [];
    userJwanAddreses.forEach(userJwanAddress => {
      promises.push(
        this.callMethod<string>(this.jwanContractStake, 'indexesForUser', [
          userJwanAddress
        ])
      );
    });
  ...
  private getDepositsForAddress(
    userJwanAddress: string,
    ...
    for (let i = 0; i < amountDeposited; i++) {
      ...
      promises.push(
        this.callMethod<IDeposit>(this.jwanContractStake, 'userDeposit', [
          userJwanAddress,
          i
        ])
      );
      ...
      Promise.all(userIndexPromises)
        .then(indexes => {
          ...
          userIndexes.forEach(item => {
            let result = this.getDepositsForAddress(
              item.address,
              item.amountDeposit
            );
        ...
        .then(res => {
          const depositsData = res.map((deposit, index) => {
            ...
            depositData.isReadyToWithdrawn =
              differenceTime > this.unixYear * 1000 ? true : false;
  ...
  public claimAll(userJwanWallets, userReward: any) {
    return new Promise<any>(async (resolve, reject) => {
      let sumUserReward = userReward.reduce(
      ...
      let feePrewiew =
        this.web3.utils.fromWei(defaultFee, 'ether') * countTransactions;
      let promises: any = [];

      const userChoise = await this.openActionSheet(
        feePrewiew,
        'claim',
        sumUserReward
      );
      ...
      userReward.forEach((reward, index) => {
        if (reward > 0) {
          promises.push(
            this.claimReward(userJwanWallets[index].wallet.linkedEthWallet)
          );
        }
      });
  ...
  public openActionSheet(feePrewiew, title: string, amount?) {
    return new Promise<any>(resolve => {
      const approveTxModal = this.actionSheetProvider.createTxConfirm({
        feePrewiew,
        title,
        amount
      });
  ...
  public getApproveAmount(userJwanAddress): Promise<string> {
    return this.callMethod<string>(this.jwanContractToken, 'allowance', [
      userJwanAddress,
      this.jwanStakeAddress
    ]);
  }
  ...
  public getApproveData(amount): string {
    return this.getWriteMetodContractData(this.jwanContractToken, 'approve', [
      this.jwanStakeAddress,
  ...
  public getDepositData(amount): string {
    const amountBg = Big(amount)
      .times(100000000)
      .toString();
    return this.getWriteMetodContractData(this.jwanContractStake, 'deposit', [
      amountBg
    ]);
  }

  public getClaimRewardData(): string {
    return this.getWriteMetodContractData(
      this.jwanContractStake,
      'claimBatch',
      []
    );
  ...
  public getUnstakeData(indexDeposit): string {
    return this.getWriteMetodContractData(this.jwanContractStake, 'withdraw', [
      indexDeposit
    ]);
  }
  ...
  public buildTxp(
    ...
    let tx = this.transactionUtilsProvider.getTx(txParam, wallet);

    return this.transactionUtilsProvider.getTxp(
      _.clone(tx),
      wallet,
      false,
      null,
  ...
  public approve(amount, walletId) {
    ...
    return this.buildTxp(walletId, dataTx, this.jwanTokenAddress, false).then(
      async txp => {
        this.onGoingProcessProvider.clear();

        const userChoise = await this.openActionSheet(
          this.web3.utils.fromWei(txp.fee, 'ether'),
          'approve',
          amount
        );
        if (userChoise) {
  ...
  public claimReward(walletId) {
    ...
    return this.buildTxp(walletId, dataTx, this.jwanStakeAddress, true).then(
      txp => {
        return this.transactionUtilsProvider.publishAndSign(txp, wallet);
      }
    );
